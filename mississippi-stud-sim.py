...
def simulate(number_of_hands, show_each_hand=False):
    """
    ...
    for i in range(number_of_hands):
        payout, hand_result, hand = simulate_hand(return_cards=True)
...
def simulate_hand(return_cards=False):
    """
...
def eval_step_1(hand):
    ...
    if rank1 == rank2:
        if rank1 >= STRATEGY_CONFIG['min_push_pair_rank']:
            return 'bet3'  # Pair of sixes or higher
        else:
            return 'bet1'  # Lower pairs
    elif card_points(card1) + card_points(card2) >= STRATEGY_CONFIG['step1_bet_threshold']:
        return 'bet1'
    else:
        return 'fold'
...
def eval_step_2(hand):
    ...
    for i in range(3):
        for j in range(i+1, 3):
            if ranks[i] == ranks[j] and ranks[i] >= STRATEGY_CONFIG['min_push_pair_rank']:
                return 'bet3'  # Pair of sixes or higher
    ...
    if card_points(card1) + card_points(card2) + card_points(card3) >= STRATEGY_CONFIG['step2_bet_threshold']:
        return 'bet1'
    else:
        return 'fold'
...
def eval_step_3(hand):
    ...
    for i in range(4):
        for j in range(i+1, 4):
            if ranks[i] == ranks[j]:
                return 'bet1'  # Any other pair
    if sum(card_points(card) for card in hand) >= STRATEGY_CONFIG['step3_bet_threshold']:
        return 'bet1'
    else:
        return 'fold'
